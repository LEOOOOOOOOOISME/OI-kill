...
(function (root) {
  ...
  function counterChain(g, trickKey, srcId, lastCounterer, depth, cont) {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      if (q.id === g.human) {
        g.pending = { type: 'counter', victim: q.id, srcId, trickKey, ctx: { type: 'counterChain', trickKey, srcId, depth, cont } };
        return 'pending';
      }
  ...
  function tryCounterOther(g, casterId, trickKey, cont, excludeId) {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      if (idx < 0 || q.mp < 1) continue;
      if (q.id === g.human) {
        g.pending = { type: 'counter', victim: q.id, srcId: casterId, trickKey, ctx: { type: 'counterChain', trickKey, srcId: casterId, depth: 0, cont } };
        return 'pending';
      }
      const c = q.hand.splice(idx, 1)[0];
      g.discard.push(c); q.mp -= 1;
  ...
  function counterAsk(g, victimId, trickKey, srcId, cont) {
    ...
    if (g.pending.ctx && g.pending.ctx.type === 'counterChain') return 'pending'; // 连锁询问已自带 cont
    if (cont && cont.ctx) g.pending.ctx = cont.ctx;
    return 'pending';
  }
  /* 由挂起特判的 ctx 重建继续信息(用于 respondCounter) */
  function contFromCtx(pd) {
    if (pd.ctx && pd.ctx.type === 'aoe') return { kind: 'aoe', ctx: pd.ctx };
    if (pd.ctx && pd.ctx.type === 'argue') return { kind: 'argue', ctx: { srcId: pd.ctx.srcId, targetId: pd.ctx.targetId, remaining: pd.ctx.remaining } };
  ...
  function runCounterCont(g, cont, countered) {
    ...
    if (cont.kind === 'aoe') {
      ...
      const r = aoeApplyOne(g, a.srcId, a.trickKey, a.dmg, a.current);
      if (r === 'pending') { g.pending.ctx = { type: 'aoe', trickKey: a.trickKey, srcId: a.srcId, dmg: a.dmg, current: a.current, remaining: a.remaining }; return; }
      resumeAoe(g, a);
    } else if (cont.kind === 'argue') {
      const ar = cont.ctx;
      if (countered) { argueStep(g, ar.srcId, ar.remaining); return; }
      const r = argueApplyOne(g, ar.srcId, ar.targetId);
      if (r !== 'pending') argueStep(g, ar.srcId, ar.remaining);
      ...
      if (countered) {
        ...
        if (hc.order[hc.pos] === g.human) { g.pending = { type: 'harvest', victim: hc.order[hc.pos], ctx: hc }; return; }
  ...
  function doTrickCore(g, srcId, trickKey, targetId) {
    ...
    switch (trickKey) {
      ...
      case 'funReport': {
        ...
        // M12: 人类选择弃哪张
        if (srcId === g.human) {
          g.pending = { type: 'report', victim: srcId, ctx: { targetId, cards: t.hand.map(c => ({ key: c.key, id: c.id, name: nsEngine.core.spec(c.key).name })) } };
          return { ok: true, result: 'pending' };
        }
        const idx = Math.floor(g.rnd() * t.hand.length);
  ...
  function resumeAoe(g, ctx) {
    for (const id of ctx.remaining) {
      ...
      const cr = counterAsk(g, id, ctx.trickKey, ctx.srcId, { kind: 'aoe', ctx: { type: 'aoe', trickKey: ctx.trickKey, srcId: ctx.srcId, dmg: ctx.dmg, current: id, remaining: rest } });
      ...
      if (r === 'pending') {
        g.pending.ctx = { type: 'aoe', trickKey: ctx.trickKey, srcId: ctx.srcId, dmg: ctx.dmg, current: id, remaining: rest };
        return;
      }
    }
  }
  /* 祖安对线(req 18.3): 依次结算两名目标(特判可反制 -> 各自选择弃1或受1伤) */
  function argueStep(g, srcId, remaining) {
    while (remaining.length) {
      const tid = remaining[0];
      const q = g.players[tid];
      if (q.dead) { remaining.shift(); continue; }
      const rest = remaining.slice(1);
      const cr = counterAsk(g, tid, 'funArgue', srcId, { kind: 'argue', ctx: { type: 'argue', srcId, targetId: tid, remaining: rest } });
      ...
      const r = argueApplyOne(g, srcId, tid);
      if (r === 'pending') {
        g.pending.ctx = { type: 'argue', srcId, trickKey: 'funArgue', targetId: tid, remaining: rest };
        return { ok: true, result: 'pending' };
      }
      remaining.shift();
    }
  ...
  function argueApplyOne(g, srcId, tid) {
    const q = g.players[tid];
    if (q.dead) return 'done';
    const p = g.players[srcId];
    // L-4: 祖安对线2目标, 不触发萌新「问问题」(需唯一目标)
    if (q.id === g.human) {
      g.pending = { type: 'argueResp', victim: tid, srcId, trickKey: 'funArgue', dmg: 1 };
      return 'pending';
    }
  ...
  function respondAoeResp(g, pid, yes) {
    const pd = g.pending;
    if (!pd || (pd.type !== 'aoeResp' && pd.type !== 'argueResp')) return { ok: false, why: '无挂起的AOE/祖安对线响应询问' };
    g.pending = null;
    const q = g.players[pid];
    const trick = pd.trickKey;
    if (pd.type === 'argueResp') {
      if (q.dead) { if (pd.ctx) argueStep(g, pd.ctx.srcId, pd.ctx.remaining); return { ok: true }; }
